...
The Pipeline Viewer is an information dense but simple window in RiderDuck. It shows all of the stateful settings of the graphics pipeline, including bound resources, rasterizer settings, etc.
...
The pipeline state viewer always displays the state of the pipeline *after* the execution of the action, as with the other viewers in RiderDuck.
...
On some APIs like Vulkan it may be common to use 'bindless' approaches where a large number of resources are bound and only a small number are accessed at any time. In these cases RiderDuck uses dynamic feedback to try to determine the set of resources accessed and only displays them. This can similarly be overridden with the Show Unused Items |page_white_delete| button.
