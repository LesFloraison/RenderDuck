...
When clicking :guilabel:`Sample Counters` in the counter selection dialog above, RiderDuck will then iterate over every event and fetch all of the data for the selected counters, and display it in a large table.
...
RiderDuck ships with support for AMD's GPA library, which will allow access to AMD's hardware counters when running with a supported AMD GPU.

RiderDuck also has support for nVidia's NvPerfKit library, but it does not ship as the library's license forbids redistribution. To enable support, place the ``NvPmApi.Core.dll`` file into either ``plugins/nv/counters/x64`` for 64-bit RiderDuck builds and ``plugins/nv/counters/x86`` for 32-bit RiderDuck builds.

Similarly RiderDuck has support for nVidia's Nsight Perf SDK, but this library's license also forbids redistribution. To enable support, follow these steps:
...
#. On windows copy ``nvperf_grfx_host.dll`` and on linux copy ``libnvperf_grfx_host.*`` to the plugins folder in ``riderduck/plugins/nv`` in your user directory.
