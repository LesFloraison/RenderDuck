...
The python shell allows flexible of RiderDuck, including the ability to run simple scripts over the whole dataset in ways not easy to do in the UI. Depending on the build-time configuration the Qt UI itself may also be exposed to python for customisation.
...
To get started the :code:`pyriderduck` object corresponds to a :py:class:`qriderduck.CaptureContext` object through which the internal API and UI windows can be obtained.
