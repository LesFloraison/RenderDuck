...
.. tip::
   To explicitly remove annotations, you can set a value with ``valueType`` equal to ``eRIDERDUCK_Empty`` and ``value`` equal to ``NULL``. This will delete the annotation at that path and all children.
...
.. code:: c++
  ...
  void AnnotateImage(ID3D11Device *dev, ID3D11Texture2D *tex, ID3D11Buffer *buf)
  {
    ...
    rdoc->SetObjectAnnotation(dev, tex, "custom.texture_flags", eRIDERDUCK_Int32, 0, RDAnnotationHelper(16));

    // referring to an object, using the temporary value structure
    RIDERDUCK_AnnotationValue val;
    val.apiObject = (void *)buf;
    rdoc->SetObjectAnnotation(dev, tex, "custom.associated_buffer", eRIDERDUCK_APIObject, 0, &val);
  }
...
.. code:: c++
  ...
  {
    void *dev = RIDERDUCK_DEVICEPOINTER_FROM_VKINSTANCE(inst);
    rdoc->SetCommandAnnotation(dev, cmd, "draw_mesh.my_property", eRIDERDUCK_Int32, 0,
                               RDAnnotationHelper(200));
                               
    rdoc->SetCommandAnnotation(dev, cmd, "draw_mesh.source_name", eRIDERDUCK_String, 0,
...
Annotations on commands can be accessed via :py:attr:`riderduck.APIEvent.annotations`, which is an optional :py:class:`~riderduck.SDObject` object that can be accessed recursively. Helper functions like :py:meth:`~riderduck.SDObject.FindChildByKeyPath` can be used to speed up accessing a specific annotation.

Annotations on objects are available in a similar way through :py:attr:`riderduck.ResourceDescription.annotations`.