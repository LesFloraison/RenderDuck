...
PyCharm does a fairly good job of code completion etc with RiderDuck's python modules, but when generating reflection for the modules it doesn't quite get everything - e.g. some parameter types and especially return types aren't properly parsed and don't get detected as the right types.
