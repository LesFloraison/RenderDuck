...
The first thing we do is enumerate a list of counters that the implementation supports using :py:meth:`~riderduck.ReplayController.EnumerateCounters`. A few of these counters values are statically known - see :py:class:`~riderduck.GPUCounter`. If you know which counter you want ahead of time you can continue straight away by calling :py:meth:`~riderduck.ReplayController.FetchCounters` with a list of counters to sample from, or :py:meth:`~riderduck.ReplayController.DescribeCounter` to obtain a :py:class:`~riderduck.CounterDescription` of the counter itself:
