...
When disassembling a shader there may be more than one possible representation available, so we first enumerate the formats that are available using :py:meth:`~riderduck.ReplayController.GetDisassemblyTargets` before selecting a target to disassemble to. The first target is always a reasonable default, and there will be at least one. :py:meth:`~riderduck.ReplayController.GetDisassemblyTargets` takes a parameter indicating whether the pipeline state object will be available - some disassembly targets require the full pipeline and are not available when disassembling only a shader in isolation:
...
For the purposes of this example we use the API abstraction :py:class:`~riderduck.PipeState` so that this code works on a capture from any API, so we fetch the state bindings that we need. Finally we fetch the disassembled shader string with :py:meth:`~riderduck.ReplayController.DisassembleShader` and print it:
...
Now we want to display the constants bound to this shader. Shader bindings is an area that diverges quite a lot between the APIs, and RiderDuck's abstraction over this is detailed in :ref:`more detail <descriptor-abstraction>`. For now, we'll simply select the first constant buffer in this shader and fetch the constants for it with :py:meth:`~riderduck.ReplayController.GetCBufferVariableContents`.
