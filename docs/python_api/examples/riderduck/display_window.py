...
# Import riderduck if not already imported (e.g. in the UI)
if 'riderduck' not in sys.modules and '_riderduck' not in sys.modules:
	import riderduck

# Alias riderduck for legibility
rd = riderduck
...
if 'pyriderduck' in globals():
	raise RuntimeError("This sample should not be run within the RiderDuck UI")
else:
	if len(sys.argv) <= 1:
