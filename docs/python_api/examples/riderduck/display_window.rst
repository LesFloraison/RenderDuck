...
Next we need to determine which windowing systems the RiderDuck implementation supports, and create a :py:class:`~riderduck.WindowingData` object for the window we want to render to. For the purposes of this example we will look for Win32 since it's the simplest to set up - needing only a window handle that we can get from tkinter easily. XCB/XLib require a display connection, which would be possible to get from another library such as Qt.

Once we have the :py:class:`~riderduck.WindowingData`, we can create a :py:class:`~riderduck.ReplayOutput` using :py:meth:`~riderduck.ReplayController.CreateOutput`.

.. highlight:: python
.. code:: python

    # Create riderduck windowing data.
    winsystems = [rd.WindowingSystem(i) for i in controller.GetSupportedWindowSystems()]

    # Pass window system specific data here, See:
    # - riderduck.CreateWin32WindowingData
    # - riderduck.CreateXlibWindowingData
    # - riderduck.CreateXCBWindowingData
...
In order to iterate over all actions we need some global state first from :py:meth:`~riderduck.ReplayController.GetTextures` and :py:meth:`~riderduck.ReplayController.GetRootActions`, and we'll also define a helper function to fetch a particular texture by resourceId, so that we can easily look up the details for a texture.
...
We now define two callback functions - ``paint`` and ``advance``. ``paint`` will be called every 33ms, it will display the output with the latest state using :py:meth:`~riderduck.ReplayOutput.Display`. ``advance`` changes the current state to reflect a new action.
...
Within ``advance`` we do a few things. First we move the current event to the current action's ``eventId``, using :py:meth:`~riderduck.ReplayController.SetFrameEvent`. Then we set up the texture display configuration with :py:meth:`~riderduck.ReplayOutput.SetTextureDisplay`, to point to the first colour output at that action.
