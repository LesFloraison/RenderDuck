...
RiderDuck exposes APIs to python at two different levels:

1. The :doc:`base replay API <riderduck/index>`, the ``riderduck`` module, which provides low level access to handling capture files, replaying frames and obtaining analysis information. The UI is built entirely on top of this API, so it provides the full power of RiderDuck, however is does not have many convenience abstractions.
2. The :doc:`RiderDuck UI API <qriderduck/index>`, the ``qriderduck`` module, which exposes the abstractions and panels within the UI tool.

Within RiderDuck - when either running scripts on the command line, or via the :doc:`../window/python_shell` - both modules are pre-imported and available automatically.

It is also possible to build the ``riderduck`` module standalone which can be loaded into python and used for scripting directly without the UI program. Due to the inherent difficulty of distributing C python modules this isn't included by default in distributed builds at the time of writing, but is generated by default in source builds - ``riderduck.pyd`` on windows or ``riderduck.so`` elsewhere. Use of this module is strictly a convenience and is not supported.
...
.. note::
    ...
    RiderDuck only supports Python 3.4+, Python 2 is not supported.
...
.. toctree::
    examples/riderduck_intro
    examples/qriderduck_intro
    dev_environment
    ui_extensions
    descriptors_bindings
    examples/basics
    examples/riderduck/index
    examples/qriderduck/index
    riderduck/index
    qriderduck/index
