...
Reference for RiderDuck in-application API version 1.6.0. This API is not necessary to use RiderDuck by default, but if you would like more control or custom triggering of captures this API can provide the mechanism to do so.

Make sure to use a matching API header for your build - if you use a newer header, the API version may not be available. All RiderDuck builds supporting this API ship the header in their root directory.

This page describes the RiderDuck API exposed to applications being captured, both in overall organisation as well as a specific reference on each function.

To begin using the API you need to fetch the ``RIDERDUCK_GetAPI`` function. You should do this dynamically, it is not recommended to actually link against RiderDuck's DLL as it's intended to be injected or loaded at runtime. The header does not declare ``RIDERDUCK_GetAPI``, it declares a function pointer typedef ``pRIDERDUCK_GetAPI`` that you can use.

The recommended way to access the RiderDuck API is to passively check if the module is loaded, and use the API if it is. This lets you continue to use RiderDuck entirely as normal, launching your program through the UI, but you can access additional functionality to e.g. trigger captures at custom times. When your program is launched independently it will see that the RiderDuck module is not present and safely fall back.

To do this you'll use your platforms dynamic library functions to see if the library is open already - e.g. ``GetModuleHandle`` on Windows, or ``dlopen`` with the ``RTLD_NOW | RTLD_NOLOAD`` flags if available on \*nix systems. On most platforms you can just search for the module name - ``riderduck.dll`` on Windows, or ``libriderduck.so`` on Linux, or ``libVkLayer_GLES_RiderDuck.so`` on Android should be sufficient here, so you don't need to know the path to where RiderDuck is running from. This will vary by platform however so consult your platform's OS documentation. Then you can use ``GetProcAddress`` or ``dlsym`` to fetch the ``RIDERDUCK_GetAPI`` function using the typedef above.

.. cpp:function:: int RIDERDUCK_GetAPI(RIDERDUCK_Version version, void **outAPIPointers)


    This function is the only entry point actually exported from the RiderDuck module. You call this function with the desired API version, and pass it the address of a pointer to the appropriate struct type. If successful, RiderDuck will set the pointer to point to a struct containing the function pointers for the API functions (detailed below) and return 1.
...
.. _riderduck-api-example:
    ...
    .. code:: c++
       ...
       #include "riderduck_app.h"

       RIDERDUCK_API_1_1_2 *rdoc_api = NULL;

       // At init, on windows
       if(HMODULE mod = GetModuleHandleA("riderduck.dll"))
       {
           pRIDERDUCK_GetAPI RIDERDUCK_GetAPI =
               (pRIDERDUCK_GetAPI)GetProcAddress(mod, "RIDERDUCK_GetAPI");
           int ret = RIDERDUCK_GetAPI(eRIDERDUCK_API_Version_1_1_2, (void **)&rdoc_api);
           assert(ret == 1);
       }

       // At init, on linux/android.
       // For android replace libriderduck.so with libVkLayer_GLES_RiderDuck.so
       if(void *mod = dlopen("libriderduck.so", RTLD_NOW | RTLD_NOLOAD))
       {
           pRIDERDUCK_GetAPI RIDERDUCK_GetAPI = (pRIDERDUCK_GetAPI)dlsym(mod, "RIDERDUCK_GetAPI");
           int ret = RIDERDUCK_GetAPI(eRIDERDUCK_API_Version_1_1_2, (void **)&rdoc_api);
           assert(ret == 1);
       }
    ...
    :param RIDERDUCK_Version version: is the version number of the API for which you want the interface struct.
    :param void** outAPIPointers: will be filled with the address of the API's function pointer struct, if supported. E.g. if ``eRIDERDUCK_API_Version_1_1_1`` is requested, outAPIPointers will be filled with ``RIDERDUCK_API_1_1_1*`` or any newer version that is compatible with API 1.1.1, but nothing lower.
...
.. cpp:function:: int SetCaptureOptionU32(RIDERDUCK_CaptureOption opt, uint32_t val)
    ...
    :param RIDERDUCK_CaptureOption opt: specifies which capture option should be set.
    :param uint32_t val: the unsigned integer value to set for the above option.
    :return: The function returns 1 if the option is valid, and the value set on the option is within valid ranges. The function returns 0 if the option is not a :cpp:enum:`RIDERDUCK_CaptureOption` enum, or the value is not valid for the option.

.. cpp:function:: int SetCaptureOptionF32(RIDERDUCK_CaptureOption opt, float val)
...
.. cpp:enum:: RIDERDUCK_CaptureOption
...
.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_AllowVSync

    specifies whether the application is allowed to enable vsync. Default is on.

.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_AllowFullscreen

    specifies whether the application is allowed to enter exclusive fullscreen. Default is on.

.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_APIValidation

    specifies whether (where possible) API-specific debugging is enabled. Default is off.

.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_CaptureCallstacks

    specifies whether each API call should save a callstack. Default is off.

.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_CaptureCallstacksOnlyActions

    specifies whether - if ``CaptureCallstacks`` is enabled - callstacks are only saved on actions. Default is off.

.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_DelayForDebugger
...
.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_VerifyBufferWrites
...
.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_HookIntoChildren
...
.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_RefAllResources
...
.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_CaptureAllCmdLists
...
.. cpp:enumerator:: RIDERDUCK_CaptureOption::eRIDERDUCK_Option_DebugOutputMute
...
.. cpp:function:: uint32_t GetCaptureOptionU32(RIDERDUCK_CaptureOption opt)

    Gets the current value of one of the different options listed above in :cpp:func:`SetCaptureOptionU32`.

    :param RIDERDUCK_CaptureOption opt: specifies which capture option should be retrieved.
    :return: The function returns the value of the capture option, if the option is a valid :cpp:enum:`RIDERDUCK_CaptureOption` enum. Otherwise returns ``0xffffffff``.

.. cpp:function:: float GetCaptureOptionF32(RIDERDUCK_CaptureOption opt)

    Gets the current value of one of the different options listed above in :cpp:func:`SetCaptureOptionF32`.

    :param RIDERDUCK_CaptureOption opt: specifies which capture option should be retrieved.
    :return: The function returns the value of the capture option, if the option is a valid :cpp:enum:`RIDERDUCK_CaptureOption` enum. Otherwise returns `-FLT_MAX`.

.. cpp:function:: void SetFocusToggleKeys(RIDERDUCK_InputButton *keys, int num)

    This function changes the key bindings in-application for changing the focussed window.

    :param RIDERDUCK_InputButton* keys: lists the keys to bind. If this parameter is ``NULL``, ``num`` must be 0.
    :param int num: specifies the number of keys in the ``keys`` array. If 0, the keybinding is disabled.

.. cpp:enum:: RIDERDUCK_InputButton

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_0

    ``eRIDERDUCK_Key_0`` to ``eRIDERDUCK_Key_9`` are the number keys. The values of these match ASCII for '0' .. '9'.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_A

    ``eRIDERDUCK_Key_A`` to ``eRIDERDUCK_Key_Z`` are the letter keys. The values of these match ASCII for 'A' .. 'Z'.


.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Divide

    is the Divide key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Multiply

    is the Multiply key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Subtract

    is the Subtract key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Plus

    is the Plus key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_F1

    ``eRIDERDUCK_Key_F1`` to ``eRIDERDUCK_Key_F12`` are the function keys.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Home

    is the Home key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_End

    is the End key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Insert

    is the Insert key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Delete

    is the Delete key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_PageUp

    is the PageUp key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_PageDn

    is the PageDn key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Backspace

    is the Backspace key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Tab

    is the Tab key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_PrtScrn

    is the PrtScrn key.

.. cpp:enumerator:: RIDERDUCK_InputButton::eRIDERDUCK_Key_Pause

    is the Pause key.

.. cpp:function:: void SetCaptureKeys(RIDERDUCK_InputButton *keys, int num)
...
.. cpp:function:: uint32_t GetOverlayBits()
    ...
    :return: A mask containing bits from :cpp:enum:`RIDERDUCK_OverlayBits`.

.. cpp:enum:: RIDERDUCK_OverlayBits

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_Enabled

    is an overall enable/disable bit. If this is disabled, no overlay renders.

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_FrameRate

    shows the average, min and max frame time in milliseconds, and the average framerate.

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_FrameNumber

    shows the current frame number, as counted by the number of presents.

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_CaptureList

    shows how many total captures have been made, and a list of captured frames in the last few seconds.

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_Default

    is the default set of bits, which is the value of the mask at startup.

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_All

    is equal to ``~0U`` so all bits are enabled.

.. cpp:enumerator:: RIDERDUCK_OverlayBits::eRIDERDUCK_Overlay_None
...
.. cpp:function:: void RemoveHooks()
    ...
    This function will attempt to remove RiderDuck and its hooks from the target process. It must be called as early as possible in the process, and will have undefined results if any graphics API functions have been called.
...
.. cpp:function:: void UnloadCrashHandler()
    ...
    This function will remove RiderDuck's crash handler from the target process. If you have your own crash handler that you want to handle any exceptions, RiderDuck's handler could interfere so it can be disabled.
...
.. cpp:function:: uint32_t IsTargetControlConnected()
    ...
    This function returns a value to indicate whether the RiderDuck UI is currently connected to the current process.

    :return: Returns ``1`` if the RiderDuck UI is currently connected, or ``0`` otherwise.
...
.. cpp:function:: uint32_t LaunchReplayUI(uint32_t connectTargetControl, const char *cmdline)
    ...
    This function will determine the closest matching replay UI executable for the current RiderDuck module and launch it.
...
.. cpp:function:: void SetActiveWindow(RIDERDUCK_DevicePointer device, RIDERDUCK_WindowHandle wndHandle)
    ...
    :param RIDERDUCK_DevicePointer device: is a handle to the API 'device' object that will be set active. Must be valid.
    :param RIDERDUCK_WindowHandle wndHandle: is a handle to the platform window handle that will be set active. Must be valid.

.. note::

    ``RIDERDUCK_DevicePointer`` is a typedef to ``void *``. The contents of it are API specific:
    ...
    * For Vulkan it must be the dispatch table pointer within the ``VkInstance``. This is a pointer-sized value at the location pointed to by the ``VkInstance``. NOTE - this is not the actual ``VkInstance`` pointer itself. You can use the ``RIDERDUCK_DEVICEPOINTER_FROM_VKINSTANCE()`` helper macro defined in the riderduck header to obtain this pointer from any VkInstance.

    ``RIDERDUCK_WindowHandle`` is a typedef to ``void *``. It is the platform specific Windows ``HWND``, Xcb ``xcb_window_t``, Xlib ``Window`` / ``Drawable``, Wayland ``wl_surface*``, or Android ``ANativeWindow*``.

.. cpp:function:: void StartFrameCapture(RIDERDUCK_DevicePointer device, RIDERDUCK_WindowHandle wndHandle)

    This function will immediately begin a capture for the specified device/window combination.

    :param RIDERDUCK_DevicePointer device: is a handle to the API 'device' object that will be set active. May be ``NULL`` to wildcard match.
    :param RIDERDUCK_WindowHandle wndHandle: is a handle to the platform window handle that will be set active. May be ``NULL`` to wildcard match.

.. note::

    ``RIDERDUCK_DevicePointer`` and ``RIDERDUCK_WindowHandle`` are described above in :cpp:func:`SetActiveWindow`.
    ...
    For example if ``device`` is ``NULL`` but ``wndHandle`` is set, RiderDuck will begin a capture on the first API it finds that is active on that window.
    ...
    If no window has been created and all rendering is off-screen, ``NULL`` can be specified for the window handle and the device object can be passed to select that API. If both are set to ``NULL``, RiderDuck will simply choose one at random so is only recommended for the case where only one is present.
...
.. cpp:function:: uint32_t EndFrameCapture(RIDERDUCK_DevicePointer device, RIDERDUCK_WindowHandle wndHandle)
...
.. cpp:function:: uint32_t DiscardFrameCapture(RIDERDUCK_DevicePointer device, RIDERDUCK_WindowHandle wndHandle)
...
.. cpp:function:: uint32_t SetObjectAnnotation(RIDERDUCK_DevicePointer device, void* object, const char *key, RIDERDUCK_AnnotationType valueType, uint32_t valueVectorWidth, const RIDERDUCK_AnnotationValue* value)
    ...
    :param RIDERDUCK_AnnotationType valueType: is the type of value to set, including basic scalar types as well as strings and API objects.
    :param uint32_t valueVectorWidth: is the vector width of the value to set, or 0 for scalars. Must be no greater than 4.
    :param const RIDERDUCK_AnnotationValue* value: is a pointer to the value itself.
    ...
    Deleting an annotation can be done by setting a value with ``valueType`` equal to ``eRIDERDUCK_Empty`` and ``value`` equal to ``NULL``. This deletes the annotation at the specific key, as well as any children of that key.
...
.. note::
    ...
    For Vulkan, annotating ``VkInstance``, ``VkPhysicalDevice``, or ``VkDevice`` objects may encounter problems due to loader wrapping. To address this, you can use ``vkSetDebugUtilsObjectTagEXT`` to set a tag with the ``tagName`` set to ``RIDERDUCK_APIObjectAnnotationHelper`` and the ``pTag`` set to the handle of the object itself. After doing that once RiderDuck will be able to recognise those handles.

    For OpenGL as it lacks proper object handles, the ``object`` parameter and any ``apiObject`` value must be a pointer to an instance of ``RIDERDUCK_GLResourceReference`` which contains both a ``GLenum`` identifier with the same meaning as the parameter in ``glObjectLabel`` as well as the integer handle itself.

.. note::

    ``RIDERDUCK_DevicePointer`` is described above in :cpp:func:`SetActiveWindow`.
...
.. cpp:enum:: RIDERDUCK_AnnotationType

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_Empty

    ``eRIDERDUCK_Empty`` is used to delete an annotation. All children are also deleted.

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_Bool

    ``eRIDERDUCK_Bool`` indicates that the ``boolean`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_Int32

    ``eRIDERDUCK_Int32`` indicates that the ``int32`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_UInt32

    ``eRIDERDUCK_UInt32`` indicates that the ``uint32`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_Int64

    ``eRIDERDUCK_Int64`` indicates that the ``int64`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_UInt64

    ``eRIDERDUCK_UInt64`` indicates that the ``uint64`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.

.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_Float

    ``eRIDERDUCK_Float`` indicates that the ``float32`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.
 
.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_Double

    ``eRIDERDUCK_Double`` indicates that the ``float64`` member of ``RIDERDUCK_AnnotationValue`` (or ``RIDERDUCK_AnnotationVectorValue``) is valid.
 
.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_String

    ``eRIDERDUCK_String`` indicates that the ``string`` member of ``RIDERDUCK_AnnotationValue`` is valid.
 
.. cpp:enumerator:: RIDERDUCK_AnnotationType::eRIDERDUCK_APIObject

    ``eRIDERDUCK_APIObject`` indicates that the ``apiObject`` member of ``RIDERDUCK_AnnotationValue`` is valid.
 
.. cpp:function:: uint32_t SetCommandAnnotation(RIDERDUCK_DevicePointer device, void* queueOrCommandBuffer, const char* key, RIDERDUCK_AnnotationType valueType, uint32_t valueVectorWidth, const RIDERDUCK_AnnotationValue* value)
...
.. note::
    ...
    For OpenGL as it lacks proper object handles, any ``apiObject`` values must be a pointer to an instance of ``RIDERDUCK_GLResourceReference`` which contains both a ``GLenum`` identifier with the same meaning as the parameter in ``glObjectLabel`` as well as the integer handle itself.
