...
.. note::
    ...
    You should choose the build of RiderDuck to match your OS - 64-bit for Windows x64 and vice-versa

    You can only capture a 64-bit process with a 64-bit build of RiderDuck. A 32-bit process can be captured by either build of RiderDuck.

RiderDuck in-app
----------------

RiderDuck has a fairly minimal in-app overlay, just to indicate that RiderDuck has successfully loaded and is ready to capture a frame.
...
.. figure:: ../imgs/QuickStart/QuickStart2.png
    ...
    The in-application RiderDuck Overlay

When the application exits, if you captured a frame it will automatically start to open in the RiderDuck UI. If you didn't capture a frame then nothing will happen and the RiderDuck UI will be back as it was when you clicked Launch.
...
RiderDuck's layout can be customised fairly heavily so this section will only cover the default layout. We'll go over the windows that are open by default and how each can be used in analysing your program.
