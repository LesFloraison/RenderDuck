...
RiderDuck supports capture and replay over a network connection. While slightly more complicated than the default workflow of doing everything on one machine, there are a number of scenarios where this can be useful.
...
If you can run the minimal RiderDuck support for capturing on a target machine such as an embedded device or devkit, the bulk of the replay work can be done on an existing machine where the UI is running.

Likewise if you have a capture that you'd like to run on a different driver or hardware, you don't have to set up a full environment to run RiderDuck, you can just run the minimal remote server.

Overview
--------

RiderDuck's network support has the core concept of a "Replay Context".
...
To add a new host, simply type its hostname in the hostname box and click ``Add``. It will be added to the list of known remote hosts, and immediately a network lookup will happen to see if the remote server or any running RiderDuck-injected applications are alive.
...
To configure such a command, enter it under the "Run Command" box and click ``Update``. If you haven't added the host yet, you can fill out both boxes at once and click ``Add``. This command runs under the context of the host with the UI, and is an arbitrary command. When the remote server is needed, RiderDuck will run this command to attempt to launch it without needing any manual intervention.

The remote server can be run with the command ``riderduckcmd remoteserver``. The exact syntax may depend on your operating system, see ``riderduckcmd remoteserver --help`` for more information, you might want to use the ``-d`` parameter to make the remote server run in the background instead of pausing interactively.
...
.. code::
    ...
    plink.exe user@host DISPLAY=:0.0 riderduckcmd remoteserver -d

Assuming that plink.exe is in ``PATH`` on the host machine, and ``riderduckcmd`` is on the host machine.
...
The drop-down will show you which remote hosts you have configured are currently already running a remote server, with periodic refreshes. When a remote host is selected, RiderDuck will attempt to connect. If it fails to find an active server but a run command is configured it will execute the command to try and launch the remote server.
...
The remote server can be configured by a file in ``~/.riderduck`` or ``%APPDATA%/riderduck`` called ``remoteserver.conf``. This allows you to restrict which IPs can connect to the server, as well as whether execution is allowed.
...
This will prevent any execution from happening under any circumstances. Note that if you do this, you will have to launch riderduck-injected commands another way and the workflow described in this document will not work as-is.
