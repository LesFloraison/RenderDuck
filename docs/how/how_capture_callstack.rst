...
This page gives an overview of the callstack-capturing capability in RiderDuck, with details and important information about how to enable this.
...
.. warning::
	...
	On Windows the callstack gathering uses ``dbghelp.dll``. If you're using this dll for some other debugging functionality in your app it is highly recommended that you disable it, otherwise it can conflict and break RiderDuck's callstack capture.
...
When launching a capture (as in :doc:`how_capture_frame`) you should enable :guilabel:`Collect callstacks`. This will set RiderDuck to collect callstacks at every API entry point that will be serialised into the capture file.
...
When the capture is loaded in RiderDuck the callstacks will be available in the API inspector. The bottom section of the dialog is a panel that can be expanded to show the callstack. Initially this section will indicate that symbols need to be resolved.
