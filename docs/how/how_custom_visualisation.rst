...
The basic process of setting up the custom shader involves writing a shader file that will be compiled and used by RiderDuck. Note that you can use any language that is either natively accepted by the graphics API used, or any language that can be compiled to an accepted shader.
...
There are several special global helpers that can be declared and used, which will be implemented and return values by RiderDuck. In addition there are automatic macros that can be used to bind resources and still write shaders which work on different APIs with different bindings.
...
To set up your shader, it's recommended that you use the UI defined in the documentation for the :doc:`../window/texture_viewer`, but you can manually create a ``.hlsl`` or ``.glsl`` file in the application storage directory ( ``%APPDATA%/qriderduck/`` on windows or ``~/.local/share/qriderduck`` elsewhere). The file must contain an entry point ``main()`` that returns ``float4``, and uses any of the below inputs. These shaders are loaded when RiderDuck loads a capture, and RiderDuck watches for any changes to the files (either externally or in the shader editor in RiderDuck) and automatically reloads them.
...
.. note::
  ...
  See the custom shader templates available in the  `contrib repository <https://github.com/baldurk/riderduck-contrib/tree/main/baldurk/custom-shader-templates>`__ for complete examples.
...
To determine which resource to sample from you can use the ``RIDERDUCK_TexType`` variable above.
