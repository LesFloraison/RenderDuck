...
#define RIDERDUCK_PLATFORM_LINUX
...
// strip off the RIDERDUCK_ namespace prefix, it's unnecessary. We list this first since we want
// any other subsequent renames to override it.
%rename("%(strip:[RIDERDUCK_])s") "";
...
// special handling for RIDERDUCK_GetDefaultCaptureOptions to transform output parameter to a return value
...
// same for RIDERDUCK_GetSupportedDeviceProtocols
...
// same for RIDERDUCK_CreateRemoteServerConnection
...
extern "C" PyObject *RIDERDUCK_DumpObject(PyObject *obj);
...
// or in qriderduck.i, depending on which one is appropriate
...
extern "C" PyObject *RIDERDUCK_DumpObject(PyObject *obj)
{
  ...
  if(PySequence_Check(obj))
  {
    ...
    for(Py_ssize_t i = 0; i < size; i++)
    {
      ...
      {
        PyObject *childDump = RIDERDUCK_DumpObject(entry);
    ...
    for(Py_ssize_t i = 0; i < size; i++)
    {
      ...
      if(PyBytes_AsStringAndSize(bytes, &buf, &size) == 0)
      {
        ...
        else
        {
          ...
          {
            PyObject *childDump = RIDERDUCK_DumpObject(child);
...
%header %{
  #include <set>
  #include "Code/pyriderduck/interface_check.h"
