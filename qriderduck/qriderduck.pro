...
# include path for core riderduck API
INCLUDEPATH += $$_PRO_FILE_PWD_/../riderduck/api/replay
...
win32 {
	...
	RC_INCLUDEPATH = $$_PRO_FILE_PWD_/../riderduck/api/replay
	RC_FILE = Resources/qriderduck.rc
	...
	swig.commands = $$_PRO_FILE_PWD_/3rdparty/swig/swig.exe -v -Wextra -Werror -O -interface ${QMAKE_FILE_BASE} -c++ -python -modern -modernargs -enumclass -fastunpack -py3 -builtin -I$$_PRO_FILE_PWD_ -I$$_PRO_FILE_PWD_/../riderduck/api/replay -outdir . -o ${QMAKE_FILE_BASE}_python.cxx ${QMAKE_FILE_IN}
	...
	SWIGSOURCES += Code/pyriderduck/riderduck.i
	SWIGSOURCES += Code/pyriderduck/qriderduck.i
	...
	LIBS += $$DESTDIR/riderduck.lib

	# Link against the version library
	LIBS += $$DESTDIR/version.lib

	QMAKE_CXXFLAGS_WARN_ON -= -w34100 
	DEFINES += RIDERDUCK_PLATFORM_WIN32

} else {
	isEmpty(CMAKE_DIR) {
		error("When run from outside CMake, please set the Build Environment Variable CMAKE_DIR to point to your CMake build root. In Qt Creator add CMAKE_DIR=/path/to/riderduck/build under 'Additional arguments' in the qmake Build Step. If running qmake directly, add CMAKE_DIR=/path/to/riderduck/build/ to the command line.")
	}
	...
	include($$CMAKE_DIR/qriderduck/qriderduck_cmake.pri)
	...
	SOURCES += $$CMAKE_DIR/qriderduck/riderduck_python.cxx
	SOURCES += $$CMAKE_DIR/qriderduck/qriderduck_python.cxx
	...
	macx: {
		...
		DEFINES += RIDERDUCK_PLATFORM_POSIX RIDERDUCK_PLATFORM_APPLE
		ICON = $$OSX_ICONFILE

		# add qrc file with qt.conf
		RESOURCES += Resources/qtconf.qrc
		
		librd.files = $$files($$DESTDIR/../lib/libriderduck.dylib)
		...
		QMAKE_POST_LINK += sh $$_PRO_FILE_PWD_/../util/set_plist_version.sh $${RIDERDUCK_VERSION}.0 $${INFO_PLIST_PATH}
	} else {
		QT += x11extras
		DEFINES += RIDERDUCK_PLATFORM_POSIX RIDERDUCK_PLATFORM_LINUX RIDERDUCK_WINDOWING_XLIB RIDERDUCK_WINDOWING_XCB
...
SOURCES += Code/qriderduck.cpp \
    ...
    Code/Resources.cpp \
    Code/RGPInterop.cpp \
    Code/pyriderduck/PythonContext.cpp \
...
HEADERS += Code/CaptureContext.h \
    ...
    Code/Resources.h \
    Code/RGPInterop.h \
    Code/pyriderduck/PythonContext.h \
    Code/pyriderduck/pyconversion.h \
    Code/pyriderduck/interface_check.h \
