...
sign_file() {
	if [ "$PRIVATE_SIGN" == "1" ]; then
		signtool.exe sign /d RiderDuck /f "${KEYFILE}" /fd sha256 /p $PASS /tr $TSS /td sha256 "${INPUTFILE}"
	elif  [ "$CLOUD_SIGN" == "1" ]; then
		timeout 5 signtool.exe sign /d RiderDuck /f "${PUBFILE}" /fd sha256          /tr $TSS /td sha256 /csp "Google Cloud KMS Provider" /kc "${KEYNAME}" "${INPUTFILE}"
	fi
