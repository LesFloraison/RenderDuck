...
if [ -d plugins-win64 ]; then
	...
	echo "WARNING: x64 plugins missing, download and extract https://riderduck.org/plugins.zip in root";
...
if [ -d plugins-win32 ]; then
	...
	echo "WARNING: x86 plugins missing, download and extract https://riderduck.org/plugins.zip in root";
...
if ls build-android-* > /dev/null; then
	find build-android-* -iname 'org.riderduck.riderduckcmd.*.apk' -exec cp '{}' dist/Release64/plugins/android ';'
else
...
# In the 64bit release folder, make an x86 subfolder and copy in riderduck 32bit
mkdir -p dist/Release64/x86
cp -R dist/Release32/{d3dcompiler_47.dll,riderduck.dll,riderduck.json,riderduckshim32.dll,riderduckcmd.exe,dbghelp.dll,symsrv.dll,symsrv.yes} dist/Release64/x86/
mkdir -p dist/ReleasePDBs64/x86
cp -R dist/ReleasePDBs32/{d3dcompiler_47.dll,riderduck.dll,riderduck.json,riderduck.pdb,riderduckshim32.dll,riderduckshim32.pdb,riderduckcmd.exe,riderduckcmd.pdb,dbghelp.dll,symsrv.dll,symsrv.yes} dist/ReleasePDBs64/x86/

VERSION=`grep -E "#define RIDERDUCK_VERSION_(MAJOR|MINOR)" riderduck/api/replay/version.h | tr -dc '[0-9\n]' | tr '\n' '.' | grep -Eo '[0-9]+\.[0-9]+'`

export RIDERDUCK_VERSION="${VERSION}"

# Ensure this variable passes through to windows on WSL
export WSLENV=$WSLENV:RIDERDUCK_VERSION
