...
VERCTRL=http
SRCSRV: variables ------------------------------------------
HTTP_ALIAS=https://raw.githubusercontent.com/baldurk/riderduck/$GITTAG/
...
for ARCH in Win32 x64; do
	...
	if [ -d "${TMPSTORE}" ]; then
		MSYS2_ARG_CONV_EXCL="*" "${BUILD_ROOT}"/support/symstore.exe add /s "$(native_path "${SYMSTORE}")" /compress /r /f "$(native_path "${TMPSTORE}")" /t RiderDuck /v $GITTAG
	fi
