...
if [ ! -d "${REPO_ROOT}"/dist/bin ]; then
	echo "Expected 'dist' folder in riderduck. When building, set CMAKE_INSTALL_PREFIX to dist and make install";
	exit 1;
fi
...
cp "./${FILENAME}"/share/doc/riderduck/{LICENSE.md,README} "./${FILENAME}"

# copy in html documentation
if [ -d "${REPO_ROOT}"/Documentation/html ]; then
	cp -R "${REPO_ROOT}"/Documentation/html "./${FILENAME}/share/doc/riderduck/html"
else
...
if [ -d "${REPO_ROOT}"/plugins-linux64 ]; then
	cp -R "${REPO_ROOT}"/plugins-linux64 "./${FILENAME}/share/riderduck/plugins"
	chmod +x -R "./${FILENAME}/share/riderduck/plugins"/*
else
...
mkdir -p "./${FILENAME}/share/riderduck/plugins/android/"

if ls "${REPO_ROOT}"/build-android*/bin/*.apk; then
	cp "${REPO_ROOT}"/build-android*/bin/*.apk "./${FILENAME}/share/riderduck/plugins/android/"
else
