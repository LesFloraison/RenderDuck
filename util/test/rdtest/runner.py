...
def run_tests(test_include: str, test_exclude: str, in_process: bool, slow_tests: bool, debugger: bool, test_timeout: int):
    ...
    log.header("Tests running for RiderDuck Version {} ({})".format(rd.GetVersionString(), rd.GetCommitHash()))
        ...
        if rd.NeedVulkanLayerRegistration(layerInfo):
            ...
            if reg_needed:
                if plat == 'win32':
                    ...
                    if 'riderduckcmd' in sys.executable:
    ...
    os.environ['RIDERDUCK_DEMOS_DATA'] = util.get_data_path('demos')
    ...
    if server is not None:
        ...
        if logfile is not None and os.path.exists(logfile):
            log.inline_file('Replay RiderDuck log', logfile)
    ...
    if os.path.exists(logfile):
        log.inline_file('{} RiderDuck log'.format("Host" if server is not None else ""), logfile)
...
def launch_remote_server():
    ...
    # if we're running from riderduckcmd, invoke it properly
    if 'riderduckcmd' in sys.executable:
        # run_tests.py
        # --riderduck
        # <riderduck_path>
...
def internal_run_test(test_name):
    ...
    for testclass in testcases:
        if testclass.__name__ == test_name:
            ...
            if logfile is not None and os.path.exists(logfile):
                log.inline_file('{} RiderDuck log'.format("Test" if server is not None else ""), logfile)
