...
parser.add_argument('-r', '--riderduck',
                    help="The location of the riderduck library to use", type=str)
parser.add_argument('-p', '--pyriderduck',
                    help="The location of the riderduck python module to use", type=str)
...
custom_pyriderduck = None

if args.pyriderduck is not None:
    if os.path.isfile(args.pyriderduck):
        custom_pyriderduck = os.path.abspath(os.path.dirname(args.pyriderduck))
    elif os.path.isdir(args.pyriderduck):
        custom_pyriderduck = os.path.abspath(args.pyriderduck)
    else:
        raise RuntimeError("'{}' is not a valid path to the pyriderduck module".format(args.pyriderduck))

if args.riderduck is not None:
    if os.path.isfile(args.riderduck):
        riderduck_dirpath = os.path.abspath(os.path.dirname(args.riderduck))
    elif os.path.isdir(args.riderduck):
        riderduck_dirpath = os.path.abspath(args.riderduck)
    else:
        raise RuntimeError("'{}' is not a valid path to the riderduck library".format(args.riderduck))
    os.environ["PATH"] += os.pathsep + riderduck_dirpath
    # Python 3.8 doesn't search PATH so add it to the DLL search path
    if sys.platform == 'win32' and sys.version_info[1] >= 8:
        os.add_dll_directory(riderduck_dirpath)

    # if the user didn't specify a pyriderduck but we do have a riderduck, try the default location as a backup
    if custom_pyriderduck is None:
        if sys.platform == 'win32':
            custom_pyriderduck = os.path.abspath(args.riderduck) + os.path.sep + "pymodules"
        else:
            custom_pyriderduck = os.path.abspath(args.riderduck)

if custom_pyriderduck is not None:
    # explicit paths go at the start, implicit paths go at the end
    if args.pyriderduck is not None:
        sys.path.insert(0, custom_pyriderduck)
    else:
        sys.path.append(custom_pyriderduck)
...
try:
    ...
    print("Couldn't import riderduck module. Try specifying path to python module with --pyriderduck " +
          "or the path to the native library with --riderduck")
    print(ex)
