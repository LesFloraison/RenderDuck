...
if uname -a | grep -qiE 'win|msys|cygwin|microsoft'; then
	...
	if docker image ls | grep -q riderduck-build; then
		echo "Building for linux";

		echo "Building riderduck-spirv-build docker image";

		pushd docker
		docker build -t riderduck-spirv-build . || exit 1
		popd

		echo "Docker image built. Running build"

		docker run --rm -v $(pwd):/script:ro -v $(pwd)/spirv-plugins-linux64:/out riderduck-spirv-build bash /script/_build.sh linux64 /out

	else
		echo "Run normal RiderDuck build first to generate riderduck-build image";
		exit 1;
	fi
fi
